...
using GhalaniDroid.CustomViews;
using System;
...
namespace GhalaniDroid.Utils
{
    ...
    internal class Validator : IDisposable
    {
        ...
        public Validator()
        {
            ...
            errorIcon = ContextCompat.GetDrawable(ctx, Resource.Drawable.error);
        }
        ...
        internal void ValidateEmail(EditText editText)
        {
            if (!alreadyFailed)
            {
                if (Regex.IsMatch(editText.Text, @"/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/1/^[a-zA-Z0-9.!#$%&’*+/=?^_`{|}~-]+@[a-zA-Z0-9-]+(?:\.[a-zA-Z0-9-]+)*$/"))
                    passed = true;
                else
                {
        ...
        /// <summary>
        /// Validates that the date in the first date picker is not later than the date in the second date picker
        /// </summary>
        /// <param name="issueDatepicker"></param>
        /// <param name="expireDatePicker"></param>
        internal void ValidateDates(Datepicker issueDatepicker, Datepicker expireDatePicker)
        {
            if (!alreadyFailed)
            {
                if (issueDatepicker.SelectedDate >= expireDatePicker.SelectedDate)
                {
                    issueDatepicker.InputLayout.SetError("Issue date cannot be later than or same as expiry date.", errorIcon);
                    issueDatepicker.InputLayout.RequestFocus();
                    passed = false;
                    alreadyFailed = true;
                }
                else
